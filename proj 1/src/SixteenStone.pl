...
fillBoard(Board, BlueStones, RedStones, Player):-
	...
	(RemainingStones =:= 0 -> playGame(Board, 2, 2, Player, [move, push, sacrifice, pass]), fail;
...
playGame(Board, BlueStones, RedStones, Player, AvailablePlays) :-
	...
	(S =:= 0 -> getNextPlayer(Player, NextPlayer), playGame(Board, BlueStones, RedStones, NextPlayer, [move, push, sacrifice, pass]);
	printBoard(Board), !,
	countPieces(Board, 0, 0, 0, 0, BS, RS),
	(BS < 2 -> nl, write('Red player wins!'), nl, fail;
	RS < 2 -> nl, write('Blue player wins!'), nl, fail; true),
	printPlayerInfo(Player, BlueStones, RedStones), nl,
	write('  Choose a play:'), nl, nl,
	printAvailablePlays(AvailablePlays, 0, S), nl,
	write('  > '),
	read(Play),
	(Play = pass -> getNextPlayer(Player, NextPlayer), playGame(Board, BlueStones, RedStones, NextPlayer, [move, push, sacrifice, pass]);
...
getPushInput(OP, X, Y, Board, ResultantBoard, Player, BlueStones, RedStones, UpdatedBs, UpdatedRs):-
	(OP =:= 1 -> validatePush(Board, X, Y, Player, [0 , -1]), move_pieces(Board, [X, Y], [0, -1], ResultantBoard, Player, BlueStones, RedStones, UpdatedBs, UpdatedRs);
	...
	nl, write('  Invalid input.'), nl, pushPiece(Board, ResultantBoard, Player, BlueStones, RedStones, UpdatedBs, UpdatedRs)).
...
printPlayerInfo(Player, BlueStones, RedStones):-
	...
	Player = 'Red' -> write(' ('), write(BlueStones), write(' stones in the pool).')), nl.
